...
from app.roles.tables import roles, permissions_in_roles
...
types = [
    PermissionTypeEnum.update, PermissionTypeEnum.delete, PermissionTypeEnum.create, PermissionTypeEnum.view
]
...
def generate_all_permission_values(resource: ResourcesEnum) -> str:
    ...
    role = conn.execute(roles.select().filter_by(title='Супер права для users')).fetchone()
    role_id = role['id']
...
def generate_read_permission_values(resource: ResourcesEnum) -> str:
    ...
    conn = op.get_bind()
    permission_ids = conn.execute(query).fetchall()
    role = conn.execute(roles.select().filter_by(title='Только чтение users')).fetchone()
    role_id = role['id']
...
def upgrade():
    ...
    query_permissions = f"INSERT INTO permissions_in_roles (role_id, permission_id) VALUES {values}"
...
def downgrade():
    ...
    read_role = session.execute(roles.select().filter_by(title='Только чтение users')).fetchone()
    ...
    super_role = session.execute(roles.select().filter_by(title='Супер права для users')).fetchone()
